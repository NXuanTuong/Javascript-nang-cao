...
import { limitPageProducts, productAll } from "../api/products";
...
const ProductPage = {
    async render() {
        ...
        if (document.location.hash.split("#/product")[1]) {
            const { data } = await limitPageProducts(document.location.hash.split("#/product")[1]);
            return /* html */ `
